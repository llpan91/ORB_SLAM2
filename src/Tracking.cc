...
cv::Mat Tracking::GrabImageMonocular(const cv::Mat& im, const double& timestamp) {
  ...
  if (mState == NOT_INITIALIZED || mState == NO_IMAGES_YET)
    ...
    mCurrentFrame =
        Frame(mImGray, timestamp, mpORBextractorLeft, mpORBVocabulary, mK, mDistCoef, mbf, mThDepth);
...
void Tracking::Track() {
  ...
  if (mState == NOT_INITIALIZED) {
    ...
    if (!mbOnlyTracking) { 	// Local Mapping is activated. This is the normal behaviour, unless you explicitly activate the "only tracking" mode.
...
bool Tracking::NeedNewKeyFrame() {
  ...
  const bool c2 =
      ((mnMatchesInliers < nRefMatches * thRefRatio || bNeedToInsertClose) && mnMatchesInliers > 15);
